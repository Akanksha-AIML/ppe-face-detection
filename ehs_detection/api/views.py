...
# FACE_DB = {}
# for name, emb_list in raw_db.items():
#     emb = np.mean(np.array(emb_list), axis=0).astype(np.float32)
#     FACE_DB[name] = emb / (np.linalg.norm(emb) + 1e-6)

# FACE_SIMILARITY_THRESH = 0.45
...
# #                 #     crop = frame[y1:y2, x1:x2]
# #                 #     stable_id = reid_tracker.get_stable_id(track_id, crop)
# #                 #     label_text = f"Person {stable_id} {confidence:.2f}"
# #                 if class_id == PERSON_CLASS_ID:
# #                     # Get ByteTrack ID
# #                     track_id = int(box.id[0]) if box.id is not None else None
...
#     return best_name if best_score >= FACE_SIMILARITY_THRESH else "Unknown"
...
#             #  ALWAYS assign color + label first
#             color, label = get_color_and_label(class_id)
...
#                             # Use stable_id if we can match a nearby person, else fall back to coords
#                             stable_id_or_track = track_id if track_id is not None else f"{x1}_{y1}"
#                             key = f"{stable_id_or_track}_{class_id}"
...
#                                 person_name = face_detector.identify_from_crop(person_crop)
#                                 save_violation_snapshot(frame.copy(), label, person_name)
#                 else:
#                     label_text = f"Person ? {confidence:.2f}"
#             # else:
...
#     # ── If violation, try to identify face and save snapshot ──
#     if class_id in VIOLATION_CLASSES:

#         now = time.time()

#         # Step 7 — cooldown (VERY IMPORTANT)
#         if now - last_saved.get(label, 0) > 10:
...
#                 person_name = identify_face(face.embedding)

#             save_violation_snapshot(frame.copy(), label, person_name)    

#             # Now safe — color always exists
#             cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
#             cv2.putText(
#                 frame,
#                 label_text,
